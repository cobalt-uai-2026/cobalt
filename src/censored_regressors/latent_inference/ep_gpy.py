import numpy as np
from GPy.util.linalg import jitchol, DSYR, dtrtrs, dtrtri, pdinv, dpotrs, tdot, symmetrify
from paramz import ObsAr
from GPy.inference.latent_function_inference import ExactGaussianInference, VarDTC
...
class gaussianApproximation(object):
    ...
    def _update_i(self, eta, delta, post_params, marg_moments, i):
        # Target based on marginal moments
        target_tau = 1. / marg_moments.sigma2_hat[i] - 1. / post_params.Sigma_diag[i]
        target_v = marg_moments.mu_hat[i] / marg_moments.sigma2_hat[i] - post_params.mu[i] / post_params.Sigma_diag[i]

        # Damping
        delta_tau = (delta / eta) * target_tau
        delta_v = (delta / eta) * target_v
...
class posteriorParamsBase(object):
    ...
    def _update_rank1(self, *arg): pass

    def _recompute(self, *arg): pass
...
class posteriorParams(posteriorParamsBase):
    ...
    def _update_rank1(self, delta_tau, delta_v, ga_approx, i):
        ...
        self.Sigma_diag = np.diag(self.Sigma)
...
class posteriorParamsDTC(posteriorParamsBase):
    ...
    def _update_rank1(self, LLT, Kmn, delta_v, delta_tau, i):
        ...
        self.Sigma_diag = np.maximum(np.sum(V * V, -2), np.finfo(float).eps)
        si = np.sum(V.T * V[:, i], -1)
...
class EPBase(object):
    def __init__(self, epsilon=1e-6, eta=1., delta=1., always_reset=False, max_iters=np.inf, ep_mode="alternated",
                 parallel_updates=False, loading=False):
    ...
    def on_optimization_end(self): pass
...
# --- STANDARD EP (For Reference/Standard Models) ---
class EP(EPBase, ExactGaussianInference):
    def inference(self, kern, X, likelihood, Y, mean_function=None, Y_metadata=None, precision=None, K=None):
        ...
        if self.ep_mode == "nested" and not self.loading:
            self._ep_approximation = None
            post_params, ga_approx, cav_params, log_Z_tilde = self._ep_approximation = self.expectation_propagation(
                mean_prior, K, Y, likelihood, Y_metadata)
        ...
        return self._inference(Y, mean_prior, K, ga_approx, cav_params, likelihood, Y_metadata=Y_metadata,
                               Z_tilde=log_Z_tilde)
    ...
    def _local_updates(self, num_data, cav_params, post_params, marg_moments, ga_approx, likelihood, Y, Y_metadata,
                       update_order=None):
        ...
        for i in update_order:
            ...
            marg_moments.Z_hat[i], marg_moments.mu_hat[i], marg_moments.sigma2_hat[i] = likelihood.moments_match_ep(
                Y[i], cav_params.tau[i], cav_params.v[i], Y_metadata_i=Y_metadata_i)
    ...
    def _log_Z_tilde(self, marg_moments, ga_approx, cav_params):
        return np.sum((
                ...
                + 0.5 * (cav_params.v * (((ga_approx.tau / cav_params.tau) * cav_params.v - 2.0 * ga_approx.v) * 1. / (
                    cav_params.tau + ga_approx.tau)))
        ))
    ...
    def _inference(self, Y, mean_prior, K, ga_approx, cav_params, likelihood, Z_tilde, Y_metadata=None):
        ...
        alpha = (ga_approx.v - tau_tilde_root * aux_alpha)[:, None]
        LWi, _ = dtrtrs(post_params.L, np.diag(tau_tilde_root), lower=1)
        Wi = np.dot(LWi.T, LWi)
        symmetrify(Wi)
        dL_dK = 0.5 * (tdot(alpha) - Wi)
        dL_dthetaL = likelihood.ep_gradients(Y, cav_params.tau, cav_params.v, np.diag(dL_dK), Y_metadata=Y_metadata,
                                             quad_mode='gh')
    ...
    def _build_from_input_dict(inference_class, input_dict):
        ...
        _ep_approximation = []
        if _ep_approximation_dict is not None:
            ...
            # Explicitly set to None if dictionary missing
            _ep_approximation = None
        ee = EP(**input_dict)
...
# --- ROBUST CENSORED EP (This is the one you must use) ---
class EPCensored(EPBase, ExactGaussianInference):
    def inference(self, kern, X, likelihood, Y, censoring, mean_function=None, Y_metadata=None, precision=None, K=None):
        if self.always_reset and not self.loading:
            self.reset()

        num_data, output_dim = Y.shape
        # assert output_dim == 1, "ep in 1D only (for now!)" # Optional check
        ...
        if self.ep_mode == "nested" and not self.loading:
            self._ep_approximation = None
            post_params, ga_approx, cav_params, log_Z_tilde = self._ep_approximation = self.expectation_propagation(
                mean_prior, K, Y, censoring, likelihood, Y_metadata)
        elif self.ep_mode == "alternated" or self.loading:
            if getattr(self, '_ep_approximation', None) is None:
                post_params, ga_approx, cav_params, log_Z_tilde = self._ep_approximation = self.expectation_propagation(
                    mean_prior, K, Y, censoring, likelihood, Y_metadata)
    ...
    def expectation_propagation(self, mean_prior, K, Y, censoring, likelihood, Y_metadata):
        num_data, data_dim = Y.shape


        Y_data = np.array(Y).copy()
        # ---------------------------------------------------------
        ...
        iterations = 0
        while not stop and (iterations < self.max_iters):
            # Pass the extracted Y_data (numpy array) to local updates
            self._local_updates(num_data, cav_params, post_params, marg_moments, ga_approx, likelihood, Y_data,
                                censoring, Y_metadata)

            # (re) compute Sigma and mu using full Cholesky decompy
            try:
                post_params = posteriorParams._recompute(mean_prior, K, ga_approx)
            except np.linalg.LinAlgError:
                print("Matrix not positive definite in recompute. Stopping EP.")
                break
    ...
    def _init_approximations(self, mean_prior, K, num_data):
        if self.ga_approx_old is None:
            v_tilde, tau_tilde = np.zeros((2, num_data))
            # Start with small positive precision to avoid singularities
            tau_tilde[:] = 1e-10
            ga_approx = gaussianApproximation(v_tilde, tau_tilde)
            Sigma = K.copy()
            diag.add(Sigma, 1e-7)
            ...
            # assert self.ga_approx_old.v.size == num_data, "data size mis-match"
            ga_approx = gaussianApproximation(self.ga_approx_old.v, self.ga_approx_old.tau)
            post_params = posteriorParams._recompute(mean_prior, K, ga_approx)
            diag.add(post_params.Sigma, 1e-7)
        return (ga_approx, post_params)

    def _local_updates(self, num_data, cav_params, post_params, marg_moments, ga_approx, likelihood, Y, censoring,
                       Y_metadata, update_order=None):
        if update_order is None:
            update_order = np.random.permutation(num_data)
        ...
        for i in update_order:
            ...
            try:
                # Moments matcher now returns LOG Z
                log_Z, mu, s2 = likelihood.moments_match_ep(Y[i], cav_params.tau[i], cav_params.v[i], censoring[i],
                                                            Y_metadata_i=Y_metadata_i)

                if np.isnan(log_Z) or np.isnan(mu) or np.isnan(s2) or s2 <= 1e-9:
                    continue

                # We store log_Z directly in Z_hat to preserve the true mathematical scale
                marg_moments.Z_hat[i] = log_Z
                marg_moments.mu_hat[i] = mu
                marg_moments.sigma2_hat[i] = s2
    ...
    def _log_Z_tilde(self, marg_moments, ga_approx, cav_params):
        # We no longer need np.log() because marg_moments.Z_hat is ALREADY log_Z!
        return np.sum((
                marg_moments.Z_hat
                + 0.5 * np.log(2 * np.pi) + 0.5 * np.log(1 + ga_approx.tau / cav_params.tau)
                - 0.5 * ((ga_approx.v) ** 2 * 1. / (cav_params.tau + ga_approx.tau))
    ...
    def _inference(self, Y, mean_prior, K, ga_approx, cav_params, likelihood, Z_tilde, Y_metadata=None):
        ...
        # Use the raw numpy array for gradients as well
        Y_data = np.array(Y).copy()

        dL_dthetaL = likelihood.ep_gradients(Y_data, cav_params.tau, cav_params.v, np.diag(dL_dK),
                                             Y_metadata=Y_metadata, quad_mode='gh')
        return Posterior(woodbury_inv=Wi, woodbury_vector=alpha, K=K), log_marginal, {'dL_dK': dL_dK,
                                                                                      'dL_dthetaL': dL_dthetaL,
                                                                                      'dL_dm': alpha}

    def to_dict(self):
        input_dict = super(EPCensored, self)._save_to_input_dict()
        input_dict["class"] = "GPy.inference.latent_function_inference.expectation_propagation.EPCensored"
        if self.ga_approx_old is not None:
            input_dict["ga_approx_old"] = self.ga_approx_old.to_dict()
        if self._ep_approximation is not None:
            input_dict["_ep_approximation"] = {}
...
class EPDTC(EPBase, VarDTC):
    def inference(self, kern, X, Z, likelihood, Y, mean_function=None, Y_metadata=None, Lm=None, dL_dKmm=None,
        ...
        if self.ep_mode == "nested" and not self.loading:
            ...
            self._ep_approximation = None
            post_params, ga_approx, log_Z_tilde = self._ep_approximation = self.expectation_propagation(Kmm, Kmn, Y,
                                                                                                        likelihood,
                                                                                                        Y_metadata)
        elif self.ep_mode == "alternated" or self.loading:
            if getattr(self, '_ep_approximation', None) is None:
    ...
    def expectation_propagation(self, Kmm, Kmn, Y, likelihood, Y_metadata):
        ...
        iterations = 0
        while not stop and (iterations < self.max_iters):
            self._local_updates(num_data, LLT0, LLT, Kmn, cav_params, post_params, marg_moments, ga_approx, likelihood,
                                Y, Y_metadata)
    ...
    def _local_updates(self, num_data, LLT0, LLT, Kmn, cav_params, post_params, marg_moments, ga_approx, likelihood, Y,
                       Y_metadata, update_order=None):
        if update_order is None:
            update_order = np.random.permutation(num_data)
        for i in update_order:
